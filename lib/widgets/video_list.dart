...
import 'package:youtube_clone/pages/video_detail.dart';
...
class _VideoListState extends State<VideoList> {
  ...
  List<YoutubeModel> listData = youtubeData;
  ...
  void initState() {
    _scrollController.addListener((){
      if(_scrollController.position.pixels == _scrollController.position.maxScrollExtent){
        setState(() {
          listData..addAll(List<YoutubeModel>.from(youtubeData));
        });
      }
    });
    super.initState();
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.isHorizontalList) {
      ...
      return ListView.separated(
        ...
        itemBuilder: (context, index) {
          if (widget.isMiniList || deviceOrientation == Orientation.landscape) {
            return InkWell(
              onTap: () {
                Navigator.of(context).push(MaterialPageRoute(
                  builder: (context) => VideoDetail(
...
//          width: MediaQuery.of(context).size.width / 2,
            width: widget.isMiniList
                ? MediaQuery.of(context).size.width / 2
                : 336.0 / 1.5,
            height: widget.isMiniList ? 100.0 : 188.0 / 1.5,
            decoration: BoxDecoration(
              image: DecorationImage(
                  image: NetworkImage(listData[index].thumbNail),
                  fit: BoxFit.cover),
          ...
          Expanded(
            child: Column(
              ...
              children: <Widget>[
                ListTile(
                  contentPadding: const EdgeInsets.all(8.0),
                  dense: widget.isMiniList ? true : false,
                  title: Padding(
                    padding: const EdgeInsets.only(bottom: 4.0),
                    child: Text(listData[index].title),
                  ),
                  subtitle: !widget.isMiniList
                      ? Text(
                          "${listData[index].channelTitle} . ${listData[index].viewCount} . ${listData[index].publishedTime}")
                      : Text(
